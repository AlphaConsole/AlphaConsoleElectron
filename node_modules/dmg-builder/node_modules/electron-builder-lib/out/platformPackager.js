...
function _load_integrity() {
    return _integrity = require("./asar/integrity");
}
...
class PlatformPackager {
    ...
    doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            yield _this2.beforeCopyExtraFiles(appOutDir, asarOptions == null ? null : yield (0, (_integrity || _load_integrity()).computeData)(resourcesPath, asarOptions.externalAllowed ? { externalAllowed: true } : null));
    ...
    copyAppFiles(taskManager, asarOptions, resourcePath, outDir, platformSpecificBuildOptions, excludePatterns, macroExpander) {
        ...
        const transformer = (0, (_fileTransformer || _load_fileTransformer()).createTransformer)(appDir, config, isElectronCompile ? Object.assign({ originalMain: this.info.metadata.main, main: (_AppFileCopierHelper || _load_AppFileCopierHelper()).ELECTRON_COMPILE_SHIM_FILENAME }, config.extraMetadata) : config.extraMetadata);
    ...
    computeAsarOptions(customBuildOptions) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            return (0, (_builderUtil || _load_builderUtil()).deepAssign)({}, result);
        })();
    }
            ...
            switch (p1) {
                ...
                default:
                    ...
                    if (p1.startsWith("env.")) {
                        ...
                        if (envValue == null) {
                            throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`cannot expand pattern "${pattern}": env ${envName} is not defined`, "ERR_ELECTRON_BUILDER_ENV_NOT_DEFINED");
                        }
                        return envValue;
                    }
                    const value = extra[p1];
                    if (value == null) {
                        throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`cannot expand pattern "${pattern}": macro ${p1} is not defined`, "ERR_ELECTRON_BUILDER_MACRO_NOT_DEFINED");
                    } else {
                        return value;
                    }
            }
    ...
    getResource(custom, ...names) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            const resourcesDir = _this6.info.buildResourcesDir;
            if (custom === undefined) {
                ...
                if ((yield (0, (_fs || _load_fs()).statOrNull)(p)) == null) {
                    p = _path.resolve(_this6.projectDir, custom);
                    if ((yield (0, (_fs || _load_fs()).statOrNull)(p)) == null) {
                        throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`cannot find specified resource "${custom}", nor relative to "${resourcesDir}", neither relative to project dir ("${_this6.projectDir}")`);
                    }
                }
                return p;
            }
    ...
    getOrConvertIcon(format) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            const iconPath = _this7.platformSpecificBuildOptions.icon || _this7.config.icon;
            if (iconPath != null) {
                const iconInfos = yield _this7.resolveIcon([iconPath], format);
            ...
            for (const fileName of sourceNames) {
                if (resourceList.includes(fileName)) {
                    return (yield _this7.resolveIcon([_path.join(resourcesDir, fileName)], format))[0].file;
                }
            }
    ...
    resolveIcon(sources, outputFormat) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            const args = ["icon", "--format", outputFormat, "--root", _this8.buildResourcesDir, "--root", _this8.projectDir, "--out", _path.resolve(_this8.projectDir, _this8.config.directories.output, `.icon-${outputFormat}`)];
            for (const source of sources) {
                args.push("--input", source);
            }
            const rawResult = yield (0, (_builderUtil || _load_builderUtil()).executeAppBuilder)(args);
            let result;
            try {
            ...
            if (errorMessage != null) {
                throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(errorMessage, result.errorCode);
            }
            return result.icons;
        })();
    }
