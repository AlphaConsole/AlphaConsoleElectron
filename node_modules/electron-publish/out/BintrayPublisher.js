...
class BintrayPublisher extends (_publisher || _load_publisher()).HttpPublisher {
    constructor(context, info, version, options = {}) {
        ...
        this._versionPromise = new (_lazyVal || _load_lazyVal()).Lazy(() => this.init());
        this.providerName = "Bintray";
        let token = info.token;
        if ((0, (_builderUtil || _load_builderUtil()).isEmptyOrSpaces)(token)) {
            token = process.env.BT_TOKEN;
            if ((0, (_builderUtil || _load_builderUtil()).isEmptyOrSpaces)(token)) {
                throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`Bintray token is not set, neither programmatically, nor using env "BT_TOKEN" (see https://www.electron.build/configuration/publish#bintrayoptions)`);
            }
            token = token.trim();
            if (!(0, (_builderUtil || _load_builderUtil()).isTokenCharValid)(token)) {
                throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`Bintray token (${JSON.stringify(token)}) contains invalid characters, please check env "BT_TOKEN"`);
            }
        }
            ...
            try {
                ...
                if (e instanceof (_builderUtilRuntime || _load_builderUtilRuntime()).HttpError && e.statusCode === 404) {
                    if (_this.options.publish !== "onTagOrDraft") {
                        (_builderUtil || _load_builderUtil()).log.info({ version: _this.version }, "version doesn't exist, creating one");
                        return _this.client.createVersion(_this.version);
                    } else {
                        (_builderUtil || _load_builderUtil()).log.notice({ reason: "version doesn't exist", version: _this.version }, "skipped publishing");
                    }
                }
                throw e;
            }
    ...
    doUpload(fileName, arch, dataLength, requestProcessor) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (version == null) {
                (_builderUtil || _load_builderUtil()).log.notice({ file: fileName, reason: "version doesn't exist and is not created", version: _this2.version }, "skipped publishing");
                return;
            }
            const options = {
            ...
            for (let attemptNumber = 0;; attemptNumber++) {
                try {
                    ...
                    if (attemptNumber < 3 && (e instanceof (_builderUtilRuntime || _load_builderUtilRuntime()).HttpError && e.statusCode === 502 || e.code === "EPIPE")) {
                        continue;
                    }
                    throw e;
                }
